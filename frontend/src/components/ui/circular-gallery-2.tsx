...
interface CircularGalleryProps extends React.HTMLAttributes<HTMLDivElement> {
  ...
  /** Fires when the carousel settles on an item (index into `items`). */
...
class App {
  ...
  scrollSpeed: number
  /**
   * `position` records where the drag started. It is not in the upstream type
   * declaration but is assigned in onTouchDown, so it is declared optional here
  ...
  onCheck() {
    ...
    // Report which room the carousel landed on, so the screen can keep its
    // detail panel in sync with what's centred.
    if (this.onSettle && this.itemCount > 0) {
      this.onSettle(((itemIndex % this.itemCount) + this.itemCount) % this.itemCount)
    }
  }
  ...
  addEventListeners() {
    ...
    // "mousewheel" is the legacy alias kept for older engines; it isn't in
    // WindowEventMap, so the handler needs a cast to satisfy the string overload.
...
const CircularGallery = ({
  ...
  }, [items, bend, borderRadius, scrollSpeed, scrollEase, fontClassName, onSettle])
